...
def fred_value(series_id):
    if not FRED_API_KEY:
        return None
    try:
        url = f"https://api.stlouisfed.org/fred/series/observations?series_id={series_id}&api_key={FRED_API_KEY}&limit=1&sort_order=desc&file_type=json"
...
def compute_gradient(d):
    ...
    if total < 20: bracket = "🟢GREEN"
    elif total < 40: bracket = "🟡전환경계"
    elif total < 60: bracket = "🟡YELLOW"
    elif total < 80: bracket = "🟠RED경계"
    else: bracket = "🔴STORM"
    return {"total": total, "bracket": bracket, "defense": defense, "attack": round(100 - defense, 1)}
...
def classify_regime(d, grad):
    ...
    return {"regime": regime, "label": label, "rp": rp,
...
def evaluate_triggers(d):
    ...
    if vix >= 30 or vv > 1.05:
        active.append({"id": "E0", "lvl": "PRE", "name": "PRE_STORM", "reason": f"VIX {vix:.1f}", "act": "공격억제"})
    if t5y > 3.0 and wti > 120:
        active.append({"id": "E1", "lvl": "PRE", "name": "INFERNO", "reason": f"T5YIE {t5y:.2f}%+WTI ${wti:.0f}", "act": "RP 20%"})
    if d["below200"] >= 5:
        active.append({"id": "E2", "lvl": "PRE", "name": "BULL_BREAK", "reason": f"200MA 하회 {d['below200']}일", "act": "매수차단"})
    br = d["BREADTH"]
    if br is not None and br >= 1.12:
        active.append({"id": "E3", "lvl": "PRE", "name": "BREADTH", "reason": f"SPY/RSP={br:.3f}", "act": "매수차단"})
    wc = d["WTI_CHG"]
    if wc is not None and wc >= 0.20:
        active.append({"id": "E4", "lvl": "PRE", "name": "ATTRITION", "reason": f"WTI 주간 {wc*100:.1f}%", "act": "매수차단"})

    # L0+
    if oas >= 5.8:
        active.append({"id": "L0a", "lvl": "L0", "name": "AEGIS-EBP", "reason": f"OAS {oas:.2f}%≥5.8%", "act": "RP 20%·축소"})
    if oas >= 5.2:
        active.append({"id": "L0b", "lvl": "L0", "name": "AEGIS-SB", "reason": f"OAS {oas:.2f}%≥5.2%", "act": "L1 승급"})
    l1r = []
    if vix >= 42: l1r.append(f"VIX {vix:.1f}≥42")
    if move >= 190: l1r.append(f"MOVE {move:.0f}≥190")
    if oas >= 8.5: l1r.append(f"OAS {oas:.2f}%≥8.5%")
    if l1r:
        active.append({"id": "L1", "lvl": "L1", "name": "STORM", "reason": " / ".join(l1r), "act": "SMH·EWZ 전량"})
    if vix >= 45:
        active.append({"id": "L2", "lvl": "L2", "name": "FAST_CRASH", "reason": f"VIX {vix:.1f}≥45", "act": "SLV·COPX·XLU"})

    ks_armed = d["below60"] >= 12 and d.get("breach60") is not None and d["breach60"] <= -0.05
    if ks_armed:
        active.append({"id": "L3", "lvl": "L3", "name": "KILLSWITCH", "reason": f"60MA 하회 {d['below60']}일", "act": "전량매도"})

    # 스테이지
    if ks_armed: stage = "L3"
    elif vix >= 45: stage = "L2"
    elif l1r: stage = "L1"
    elif any(t["lvl"] == "L0" for t in active): stage = "L0"
    elif any(t["lvl"] == "PRE" for t in active): stage = "PRE"
    else: stage = "CLEAR"
    ...
    return {"active": active, "emergency": emergency, "stage": stage, "gld_prohibited": gld_no}
...
def send_status(d, regime, grad, trig):
    ...
    stg = trig["stage"]
    emoji = {"CLEAR": "🟢", "PRE": "🟡", "L0": "🟠", "L1": "🔴", "L2": "🔴🔴", "L3": "⚫"}.get(stg, "⚪")
    ...
    fields = [
        ...
        {"name": "📈 그래디언트", "value": f"**{grad['total']}**/100 {grad['bracket']}", "inline": True},
        ...
        {"name": "🥇 GLD", "value": "🚫금지" if trig["gld_prohibited"] else "✅허용", "inline": True},
        {"name": "🛡️방어/⚔️공격", "value": f"{grad['defense']}% / {grad['attack']}%", "inline": True},
        {"name": "TIER1",
         "value": f"VIX **{fmt(d['VIX'])}** · MOVE **{fmt(d['MOVE'],dec=0)}** · OAS **{fmt(d['OAS'],'',2)}%**\nWTI **${fmt(d['WTI'])}** · T5YIE **{fmt(d['T5YIE'],'',2)}%** · DXY **{fmt(d['DXY'],dec=2)}**",
         "inline": False},
        {"name": "TIER2",
         "value": f"DFII10 **{fmt(d['DFII10'],'',2)}%** · T10Y2Y **{fmt(d['T10Y2Y'],'',2)}%** · SAHM **{fmt(d['SAHM'],'',2)}** · ICSA **{fmt(d['ICSA'],'',0)}**",
         "inline": False},
        {"name": "트리거", "value": chips, "inline": False},
    ]

    send_discord({"embeds": [{"title": f"🛡️ INVICTUS — {emoji} {stg}", "color": color, "fields": fields, "footer": {"text": now}}]})
...
def send_alert(d, regime, grad, trig):
    ...
    send_discord({
        ...
        "embeds": [{"title": f"⚠️ 경보: {trig['stage']}", "description": "\n\n".join(lines), "color": 0xFF0000, "footer": {"text": now}}]
    })
