...
import os,requests,xml.etree.ElementTree as ET,math
...
# 보유종목 — 변경 시 여기만 수정
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
TICKERS = ["GLD","SMH","EWZ","XLE","SLV","PAVE","COPX","XLU","VEA","SPY"]
EMOJIS  = {"GLD":"🥇","SMH":"📱","EWZ":"🇧🇷","XLE":"🛢️","SLV":"🥈","PAVE":"🏗️","COPX":"🟤","XLU":"⚡","VEA":"🌍","SPY":"📈"}
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def build(d,reg,grd,trg):
    ...
    e1={"title":f"☀️ INVICTUS 모닝 브리핑 — {date}","color":sc,"description":(
        ...
        f"{vt_dot} **공격예산** {vt_pct}% (SPY 20일 변동성 기반 자동축소)"
    )}
    ...
    # 5️⃣ 보유종목 모멘텀 (Legio mom_score 순위)
    ranked=[]
    for tk in TICKERS:
        h=d["H"].get(tk,{})
        sc=h.get("score")
    ...
    lines=[]
    for i,(tk,h,scv) in enumerate(ranked):
        em=EMOJIS.get(tk,"")
        ...
        # 연속하락 경고
        cd_str=f" │ ⚠️{cd}일연속↓" if cd>=3 else""
        lines.append(
            f"**#{i+1}** {sc_dot} {em}{tk} **{sc_str}** │ "
            f"${f(p)} │ 1D {sg(d1)} │ 1M {sg(m1)}\n"
            f"　　 {rsi_dot} RSI **{f(rsi,dc=0)}** │ "
            f"{w52_dot} 52주 **{f(w52)}%**{cd_str}"
        )
    e5={"title":"📊 보유종목 Legio 모멘텀 순위","color":0x3DBB7E,"description":(
        "\n".join(lines)
    )}
    ...
    e7={"color":0x485070,"description":(
        ...
        "▸ **모멘텀 신호등** 🟢+5%↑ 🟡0~5% 🟠-5~0% 🔴-5%↓\n"
        "▸ **Legio score** 가중모멘텀(1M25+3M30+6M30+12M15) × vol감쇠 × MA20감쇠\n"
        "▸ **RSI** 14일 상대강도. 🟢40~60 🟡60~70/30~40 🔴70↑과매수 30↓과매도\n"
        "▸ **52주** 52주 최고가 대비 %. 🟢-3%내 🟡-10%내 🔴-10%↓\n"
        "▸ **연속↓** 3일 이상 연속하락 시 ⚠️경고\n"
        "▸ **공격예산** SPY 20일 변동성 기반 자동축소 (Moreira&Muir 2017)\n"
        "▸ **진입게이트** DXY/VIX 기반 SLV·COPX·VEA 매수허용 비율"
