...
패치 이력:
    - 2026-04-21 v4.5: Rich 카드형 한경·외신 통합.
                      ...
                      Claude Haiku JSON-mode, 각 기사 1 embed.
                      기존 v4.4 단순 통합 embed 대체.
    - 2026-04-21 v4.4: 한경 RSS 뉴스 통합 (https://www.hankyung.com/feed).
                      fetch_hankyung_news / summarize_hankyung 신규.
                      외신+한경을 단일 embed "경제 뉴스 종합"으로 통합.
    - 2026-04-21 v4.3: Top10 섹션 시인성 개선.
                      2줄/종목 → 1줄/종목, 점수 막대 `█░` 10칸 추가,
                      가격·1D% 제거하여 한눈 비교 최적화.
                      수식 불변, 표시 레이아웃만 변경.
...
def fetch_hankyung_news():
    """한경 RSS에서 헤드라인 수집 (최대 15개)."""
    headlines = []
    try:
        r = requests.get(HANKYUNG_RSS, headers=UA, timeout=10)
        root = ET.fromstring(r.content)
        ...
        _report("hankyung:feed", e)
    return headlines


def summarize_hankyung(headlines):
    """한경 헤드라인을 Claude Haiku로 5~7개 요약 (v4.4 유지용, v4.5는 rich 사용).

    투자·시장·정책 관점에서 중요한 항목만 선별, 광고·연예·중복 제거.
    """
    if not ANTHROPIC_API_KEY or not headlines:
        return None
    try:
        ...
        r = requests.post(
            ...
            json={
                ...
                "messages": [{
                    "role": "user",
                    "content": (
                        "아래 한국 경제 뉴스 헤드라인에서 중복·광고·연예·가십을 제거하고 "
                        "투자·시장·정책 관점에서 중요한 5~7개를 선별하라. "
                        "각 항목은 ▸ 로 시작하는 1줄 요약(원문 표현 유지 가능). 출처 불필요.\n\n"
                        f"{joined}"
                    ),
                }],
            },
        ...
        print(f" 한경 Claude: {r.status_code}")
        if r.status_code != 200:
            print(f" 응답: {r.text[:300]}")
            return None
        return r.json().get("content", [{}])[0].get("text", "").strip() or None
    except Exception as e:
        _report("anthropic:hankyung", e)
        print(f" 한경 요약 에러: {e}")
        return None


# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# v4.5 NEW: Rich 카드형 뉴스 (한경 + 외신 3사 교차검증)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

GLOBAL_RSS_FEEDS = [
    ("https://www.cnbc.com/id/20910258/device/rss/rss.html", "CNBC"),
    ("https://feeds.bbci.co.uk/news/business/rss.xml", "BBC"),
    ("https://www.theguardian.com/business/rss", "Guardian"),
]

HANKYUNG_COLOR_MAP = {
    "통화정책": 0x3498DB, "에너지": 0xE67E22, "산업": 0x95A5A6,
    "부동산": 0x16A085, "금융": 0x9B59B6, "기업": 0x3498DB,
    "정책": 0xF1C40F, "IT": 0x2ECC71, "자동차": 0xE74C3C,
    "AI": 0x2ECC71, "반도체": 0x2ECC71, "기타": 0x95A5A6,
}

GLOBAL_COLOR_MAP = {
    "지정학": 0xE74C3C, "기업·매크로": 0x3498DB, "AI/반도체": 0x2ECC71,
    "통화": 0xF1C40F, "에너지": 0xE67E22, "무역": 0x9B59B6,
    "기타": 0x95A5A6,
}


def _strip_html(text, max_len=500):
    """RSS description에서 HTML·CDATA 제거 후 길이 제한."""
    if not text:
        return ""
...
def fetch_global_articles():
    """CNBC + BBC + Guardian 통합 수집."""
    all_articles = []
    for url, name in GLOBAL_RSS_FEEDS:
        all_articles.extend(fetch_rss_articles(url, name, max_items=10))
    return all_articles


def parse_claude_json(text):
    """Claude 응답에서 JSON 배열 추출. markdown fence·주변 텍스트 허용."""
    if not text:
        return None
    text = text.strip()
    # ```json ... ``` 또는 ``` ... ``` 제거
...
_HANKYUNG_PROMPT = (
    "다음은 한국경제 RSS 최근 기사의 제목+설명입니다.\n"
    "경제·금융·정책·산업·기업 관점에서 중요한 5개 토픽을 선별해 JSON 배열로 반환하라.\n\n"
    "각 항목 스키마:\n"
    "- no: 1~5 정수\n"
    "- icon: 이모지 1자 (🏛️ 통화 | 🛢️ 에너지 | 🏗️ 산업 | 🏠 부동산 | 🏦 금융 | "
    "💼 기업 | 📜 정책 | 💻 IT | 🚗 자동차 | 🤖 AI | 🔬 반도체 중)\n"
    "- category: 통화정책/에너지/산업/부동산/금융/기업/정책/IT/자동차/AI/반도체\n"
    "- title: 한국어 1줄 제목\n"
    "- conclusion: 결론 1~2문장 (핵심 압축, 한국어)\n"
    "- details: 한국어 bullet 문자열 배열 3~6개 (각 1줄)\n\n"
    "규칙: 광고·연예·가십·중복 제외. 헤드라인이 부족하면 일반 맥락 보충 가능. "
    "JSON 외 텍스트 금지. 순수 배열만 반환.\n\n"
    "기사 (제목 :: 설명 :: 출처):\n"
)

_GLOBAL_PROMPT = (
    "다음은 CNBC·BBC·Guardian 3개 매체 최근 비즈니스 기사의 제목+설명입니다.\n"
    "경제·정치·기업·기술·지정학 관점에서 중요한 5개 토픽을 선별, 한국 투자자 관점에서 분석하라.\n"
    "같은 사건이 여러 매체에 있으면 '교차 검증'으로 표시.\n"
    "출력은 모두 한국어.\n\n"
    "각 항목 스키마 (JSON 배열로):\n"
    "- no: 1~5 정수\n"
    "- icon: 이모지 1자 (🔴 지정학 | 💼 기업 | 🤖 AI | 🏛️ 통화 | 🛢️ 에너지 | 📦 무역 중)\n"
    "- category: 지정학/기업·매크로/AI·반도체/통화/에너지/무역\n"
    "- title: 한국어 1줄 제목 (영어 키워드 포함 가능)\n"
    "- core: 핵심 요약 2~4문장 (한국어). Fed·트럼프·한국 등 영향 명시\n"
    "- implications: 한국 투자자 관점 투자 시사 1~3문장 (한국어)\n"
    "- source_cross: 'X·Y·Z N개 교차' 형식 (1개만이면 'CNBC 1개' 같이)\n"
    "- primary_source: CNBC | BBC | Guardian 중 가장 강한 한 곳\n\n"
    "규칙: 실제 교차 확인된 매체만 source_cross에 포함. "
    "JSON 외 텍스트 금지. 순수 배열만 반환.\n\n"
    "기사 (제목 :: 설명 :: 출처):\n"
)


def generate_rich_briefing(articles, kind):
    """Claude로 rich briefing JSON 생성. kind: 'hankyung' | 'global'.

    실패 시 None 반환 → 호출자가 fallback 결정.
    """
    if not ANTHROPIC_API_KEY or not articles:
        return None

    if kind == "hankyung":
        prompt_head = _HANKYUNG_PROMPT
    elif kind == "global":
        prompt_head = _GLOBAL_PROMPT
    else:
        return None

    # 기사 목록을 프롬프트에 포함 (길이 제한)
    lines = []
    for a in articles:
    ...
    formatted = "\n".join(lines[:30])  # 최대 30개 기사

    prompt = prompt_head + formatted
    ...
    try:
        ...
        print(f" Rich {kind}: HTTP {r.status_code}")
        if r.status_code != 200:
            print(f" 응답: {r.text[:300]}")
            return None
        text = r.json().get("content", [{}])[0].get("text", "")
        items = parse_claude_json(text)
        if items is None:
            print(f" {kind} JSON 파싱 실패. 원본: {text[:300]}")
            return None
        if not isinstance(items, list):
            print(f" {kind} 배열 아님: {type(items).__name__}")
            return None
        return items
    except Exception as e:
        _report(f"anthropic:rich:{kind}", e)
        print(f" rich 생성 에러 ({kind}): {e}")
        return None


def hankyung_items_to_embeds(items):
    """한경 JSON items → Discord embed list."""
    embeds = []
    for it in items:
        ...
        color = HANKYUNG_COLOR_MAP.get(category, 0x95A5A6)
        title = str(it.get("title", "")).strip()
        conclusion = str(it.get("conclusion", "")).strip()
        ...
        desc_parts = []
        if conclusion:
            desc_parts.append(f"• **결론:** {conclusion}")
        for d in details:
            desc_parts.append(f"• {str(d).strip()}")
...
def global_items_to_embeds(items):
    """외신 JSON items → Discord embed list."""
    embeds = []
    for it in items:
        if not isinstance(it, dict):
            continue
        ...
        icon = it.get("icon", "🌐")
        category = it.get("category", "기타")
        color = GLOBAL_COLOR_MAP.get(category, 0x95A5A6)
        title = str(it.get("title", "")).strip()
        core = str(it.get("core", "")).strip()
        implications = str(it.get("implications", "")).strip()
        source_cross = str(it.get("source_cross", "")).strip()
        primary = str(it.get("primary_source", "")).strip() or "외신"

        desc_parts = []
        if core:
            desc_parts.append(f"**핵심:** {core}")
        if implications:
            desc_parts.append(f"**투자 시사:** {implications}")
        if source_cross:
            desc_parts.append(f"_(출처: {source_cross})_")
        ...
        embeds.append({
            ...
            "color": color,
            "description": description,
            "footer": {"text": f"{primary} · {category}"},
        })
    return embeds
...
def build(d, reg, grd, trg, hist_eff_atk=None):
    ...
    # 5️⃣ 보유종목 모멘텀 Top10 (v4.5: 막대 제거 + 종가·1D 복원, v4.2 형식)
    ranked = []
    for tk in TICKERS:
        h = d["H"].get(tk, {})
        scv = h.get("score")
    ...
    for i, (tk, h, _scv) in enumerate(ranked[:10]):
        ...
        # v4.2 형식: 2줄/종목 — 종가 + 1D 변동율 + RSI/52주 신호등
        lines.append(
...
def main():
    ...
    # 코어 INVICTUS 브리핑 (e1~e7) 발송 — v4.5는 뉴스를 별도 메시지로 분리
    now = datetime.now(KST)
    ok1 = send({
    ...
    if ok1 and ok_core2:
        ...
        print(f" ⚠️ 코어 브리핑 부분 실패 "
              f"(첫 메시지 {'ok' if ok1 else 'fail'}, "
              f"두 번째 {'ok' if ok_core2 else 'fail'})")
