...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
EVENTS={
    "2026-04-22":"🚨 이란 휴전 만료 (D-day)",
    "2026-05-04":"⚠️ AI_POWER 재승인 마감 (D+14)",
    "2026-05-15":"🏛️ Warsh Fed 취임",
    "2026-07-24":"📋 Section 122 만료",
}
def get_event_alerts(window=7):
    """오늘 기준 -1~+window일 사이 이벤트 반환"""
    today=datetime.now(KST).date()
    alerts=[]
    for ds,label in sorted(EVENTS.items()):
        try:
            d=datetime.strptime(ds,"%Y-%m-%d").date()
            days=(d-today).days
            if -1<=days<=window:
                sign="+" if days>=0 else ""
                alerts.append(f"{label} (D{sign}{days})")
        except Exception as e:
            if os.environ.get("DEBUG"):print(f"[event parse fail] {ds}: {e}")
    return alerts
...
SCENARIO_MAP = {
    "S1": {"label":"🕊️ 급속해결","method":"weighted","markets":[
        # 호르무즈 해협 정상화 = S1 핵심 지표 (april은 만기 임박 제외, 월1회 갱신 권장)
        {"slug":"strait-of-hormuz-traffic-returns-to-normal-by-end-of-may","dir":"YES","w":0.30},
        {"slug":"strait-of-hormuz-traffic-returns-to-normal-by-end-of-june","dir":"YES","w":0.20},
        # 침체 회피 = S1 확증
        {"slug":"us-recession-by-end-of-2026","dir":"NO","w":0.50},
    ]},
...
def yp(s):
    try:return requests.get(f"https://query2.finance.yahoo.com/v8/finance/chart/{requests.utils.quote(s)}?interval=1d&range=1d",headers=UA,timeout=10).json()["chart"]["result"][0]["meta"]["regularMarketPrice"]
        ...
        if os.environ.get("DEBUG"):print(f"[bare] {_e}")
...
def fetch():
    ...
    for tk in TICKERS:
        ...
        hdata[tk]={"p":p,"ma50":ma50,"1D":mom(h,1),"1M":mom(h,22),"3M":mom(h,63),"6M":mom(h,126),"12M":mom(h,252),"score":ms,"rsi":rsi,"w52":w52,"cdrops":cdrops}
...
def build_oracle_embed(probs,raw_markets,status):
    ...
    # 🚨 Polymarket vs 공식 SSOT 괴리 경고
    gaps=compare_ssot(probs)
    warn_block=""
    if gaps:
        warn_block="\n🚨 **Polymarket vs 공식 SSOT 괴리**\n"+"\n".join(f"▸ {g}" for g in gaps)+"\n"
    desc=(
...
def collect_solidus(vix_override=None):
    """SOLIDUS 전체 데이터 수집 + 스코어링 + 목표비중 계산.
    vix_override: INVICTUS가 이미 수집한 VIX 값. 있으면 재사용해 Yahoo 중복 호출 회피."""
    btc=fetch_btc_price()
    fng=fetch_fng()
    dxy5d=fetch_dxy_change_5d()
    svix=vix_override if vix_override is not None else fetch_sol_vix()
    mvrv=fetch_mvrv()
    ...
    return{"target":target,"price":btc["price"],"change_24h":btc["change_24h"],
           ...
           "_vix_source":"invictus_reused" if vix_override is not None else "yahoo_fresh"}
...
def build(d,reg,grd,trg,oracle):
    ...
    # §1.5 공식 SSOT + Top7 목표비중
    e1_5=build_ssot_embed(d)
    ...
    return[e1,e1_5,e2,e3,e3_5,e4,e5,e6,e7]
...
def main():
    ...
    try:
        sd=collect_solidus(vix_override=d.get("VIX"))
        sol_embeds=build_solidus_embeds(sd)
        embeds.extend(sol_embeds)
        print(f"  🟠 SOLIDUS embed 2개 (target={sd['target']:.1f}%, 지표 {sd['n_indicators']}/5, VIX={sd.get('_vix_source','?')})")
    except Exception as e:
    ...
    # D-day 이벤트 알림
    alerts=get_event_alerts(window=7)
    alert_line=("\n### 📅 "+" · ".join(alerts)) if alerts else ""
    header=f"# 🛡️ INVICTUS 모닝 리포트 ☕\n## {now.strftime('%Y-%m-%d %H:%M KST')}{alert_line}"
    # Part별 명시적 분할 (논리 경계 유지)
    # embeds=[§1, §1.5, §2, §3, §3.5, §4, §5, §6, 각주, §7 SOL, §8 SOL, §9 명언]
    parts=[embeds[0:5], embeds[5:9], embeds[9:]]
    for i,part_embeds in enumerate(parts):
        if not part_embeds:continue
        payload={"embeds":part_embeds}
        if i==0:payload["content"]=header
        send(payload)
    total_e=sum(len(p) for p in parts)
    print(f"  ✅ 전송 완료 ({total_e}개 / Part1:{len(parts[0])} Part2:{len(parts[1])} Part3:{len(parts[2])})")
