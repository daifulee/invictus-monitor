...
def build(d, reg, grd, trg, hist_eff_atk=None):
    ...
    # 5️⃣ 보유종목 모멘텀 Top10 (v4.3: 1줄/종목 + 점수 막대)
    ranked = []
    for tk in TICKERS:
        h = d["H"].get(tk, {})
        scv = h.get("score")
    ...
    # 막대 스케일: #1의 점수 기준으로 비례. 양수 점수만 막대 표시.
    top10 = ranked[:10]
    positive_scores = [s for _, _, s in top10 if s is not None and s > 0]
    max_pos = max(positive_scores) if positive_scores else 0.0

    lines = []
    for i, (tk, h, _scv) in enumerate(top10):
        em = EMOJIS.get(tk, "")
        score = h.get("score")
        rsi = h.get("rsi")
        w52 = h.get("w52")
        ...
        if score is None:
            ...
            sc_str = f"{score:+.3f}"
            if score > 0 and max_pos > 0:
                bar_len = max(0, min(10, int(round(score / max_pos * 10))))
            else:
                bar_len = 0
        ...
        # 1줄 포맷: #N 🥇 GLD `██████████` +0.135 │ 58🟢 -2.1%🟢
        rsi_str = f"{f(rsi,dc=0)}{rsi_dot}" if rsi is not None else "--"
        w52_str = f"{f(w52)}%{w52_dot}" if w52 is not None else "--"

        lines.append(
            f"**#{i+1:<2}** {em} **{tk}**  `{bar}` **{sc_str}** │ "
            f"{rsi_str}  {w52_str}{cd_str}"
        )
    ...
    e5 = {
        ...
        "color": 0x3DBB7E,
        "description": (
            "_막대: #1 대비 상대강도 · 숫자: 점수 · RSI · 52주 고점대비_\n"
            + "\n".join(lines)
        ),
    }
    ...
    e7 = {
        ...
        "description": (
            ...
            "▸ **Top10 막대** `█░` 10칸. #1 점수 기준 비례. 양수만 표시 (음수는 0칸)"
        ),
        "footer": {
